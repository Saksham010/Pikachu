...
fn load_key_from_file(file_name:&str,g1g2_seperator_index:u8) -> Result<Vec<Vec<ProjectiveConfigType>>>{
    ...
    while (cursor.position() as usize) < cursor.get_ref().len(){ //Ignoring the last 0 delimiter
        ...
        if g1g2_seperator_index != 0 && iteration_no >= g1g2_seperator_index {
...
fn load_key_from_file_g2(file_name:&str) -> Result<Vec<Vec<G2>>>{
    let mut file = File::open(file_name).unwrap();

    let mut final_key:Vec<Vec<Projective<ark_bn254::g2::Config>>> = Vec::new();
    let mut key_vec_inner:Vec<Projective<ark_bn254::g2::Config>> = Vec::new();
    ...
    while (cursor.position() as usize) < cursor.get_ref().len(){ //Ignoring the last 0 delimiter
        ...
        let mut cursory = Cursor::new(y_element);
        let mut cursorz = Cursor::new(z_element);
        // QuadExtField<Fp2ConfigWrapper<Fq2Config>>
...
fn main(){
    ...
    //Read proving key
    let proving_key = load_key_from_file("proving_key.bin",0).unwrap();
    ...
    //Evaluations
    let gsk = proving_key[0].clone();
    let gl_left_operand_poly_eval = proving_key[1].clone();
    let gr_right_operand_poly_eval = proving_key[2].clone();
    let go_output_operand_poly_eval = proving_key[3].clone();
    let gl_alpha_left_operand_poly_eval = proving_key[4].clone();
    let gr_alpha_right_operand_poly_eval = proving_key[5].clone();
    let go_alpha_output_operand_poly_eval = proving_key[6].clone();
    let gl_beta_left_operand_poly_eval = proving_key[7].clone();
    let gr_beta_right_operand_poly_eval = proving_key[8].clone();
    let go_beta_output_operand_poly_eval = proving_key[9].clone();
    let g_vanishing_eval = proving_key[10].clone();
    ...
    let gr_t_eval_deltar = gr_t_eval.mul(delta_r); // gl^t(s)^deltal
    let go_t_eval_deltao = go_t_eval.mul(delta_o); // gl^t(s)^deltal

    
    let gl_alphal_t_eval:Projective<Config> = extract_g1_element(g_vanishing_eval[3]); //gl^alphal*t(s)
    let gr_alphal_t_eval:Projective<Config> = extract_g1_element(g_vanishing_eval[4]); //gr^alphal*t(s)
    let go_alphal_t_eval:Projective<Config> = extract_g1_element(g_vanishing_eval[5]); //go^alphal*t(s)

    let gl_alphal_t_eval_deltal = gl_alphal_t_eval.mul(delta_l); //gl^alphal*t(s)^deltal
    let gr_alphal_t_eval_deltar = gr_alphal_t_eval.mul(delta_r); //gr^alphal*t(s)^deltal
    let go_alphal_t_eval_deltao = go_alphal_t_eval.mul(delta_o); //go^alphal*t(s)^deltal
    ...
    //Compute gr^R'p(s)
    let gr_rop_shifted_eval =  compute_encrypted_polynomial_evaluation(gr_alpha_right_operand_poly_eval.clone(),right_occurance_list.clone(),gr_alphal_t_eval_deltar,witness_values.clone());
    ...
    //Compute go^O'p(s)
    let go_oop_shifted_eval = compute_encrypted_polynomial_evaluation(go_alpha_output_operand_poly_eval.clone(),output_occurance_list.clone(),go_alphal_t_eval_deltao,witness_values.clone());

    
    //Fetch g and g alphal from verification key and test
    let generator_g2 = extract_g2_element(verification_key[5][0].clone());
    ...
    let left_part_pairing_r = Bn254::pairing(gr_rop_eval, g_alphar_g2);
    let right_part_pairing_r = Bn254::pairing(gr_rop_shifted_eval, generator_g2);
    ...
    // println!("Left pairing part(L): {:?}",left_part_pairing_l);
    // println!("");
    // println!("Right pairing part(L): {:?}",right_part_pairing_l);

    
    // println!("Left pairing part(R): {:?}",left_part_pairing_r);
    // println!("");
    // println!("Right pairing part(R): {:?}",right_part_pairing_r);

    
    // println!("Left pairing part(O): {:?}",left_part_pairing_o);
    // println!("");
    // println!("Right pairing part(O): {:?}",right_part_pairing_o);
